...
├── TRADEMARKS.md           AIPARC name and organisation logos
...
**The laboratory name AIPARC, that identity, and the organisation logos are not licensed by MIT.** Do not use them as the brand of a fork, service, or product, or to imply an official laboratory build. Factual statements of origin (“derived from AIPARC ETA”) are fine. Details: [`TRADEMARKS.md`](../TRADEMARKS.md).

`"private": true` in `package.json` only means do not publish to the npm Registry. It is not a copyright statement.

Third-party works keep their own terms: npm packages as declared in each `license` field; the PP-OCRv6 small ONNX weights in `ocr/models/` come from [PaddleOCR](https://github.com/PaddlePaddle/PaddleOCR) under [Apache-2.0](https://www.apache.org/licenses/LICENSE-2.0). `.env`, database contents, and Discord tokens are out of scope and must not be committed.
